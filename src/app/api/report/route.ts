...
2. Commission's Detailed Observations and Reasoning 
   (Detailed bullet points for EVERY item. For each item, clearly explain the reason for the verdict based on the provided summary. Use ₹ symbol for currency. DO NOT create any tables here, only text and bullet points.)
3. Summary Order / Directions

CRITICAL INSTRUCTION: DO NOT generate ANY tables in your response. The tables (Component-wise Analysis, Revenue Gap, Controllable/Uncontrollable) are generated by the system automatically. Your job is ONLY to write the narrative text.`;
}
...
export async function POST(req: NextRequest) {
    try {
        ...
        const validCostHeads = costHeads?.filter((h: any) => Number(h.actual_cr) !== 0 || Number(h.approved_cr) !== 0);

        const costHeadsSummary = validCostHeads?.map((h: any) =>
